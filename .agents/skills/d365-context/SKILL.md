...
4. `/skill:d365-deploy` — 发布部署指南（n8n、Assembly 更新、Publish 规范）
5. `/skill:d365-dev` — 开发规范与流程（命名规范、状态值、远程服务器、Git 工作流）
6. `/skill:d365-system-access` — 环境地址、账号、工具认证

> **注意**：专项 skill 会同步更新 AI 的红线与操作边界，必须在恢复上下文时一并读取，不能只读 `Memory.md`。
...
> **⚠️ 若用户要启动新实体/新模块开发**：必须先执行 `/skill:d365-dev` 第 9 章「实体/模块开发前核对流程」：重新读取 PRD → 核对字段定义 → 核对业务逻辑 → 整理开发思路 → 用户确认无误后再编码。**不得直接写代码。**
