...
- **🚨 新增组件必须及时加入主清单 Solution（2026-07-20 新增）**：`AllComponent_Peter_NoUAT`（显示名「AllComponent_Peter_整理_禁止导入UAT」）是发版分布核对的唯一基准，禁止导入 UAT。开发在 DEV1 新增任何 D365 组件（实体/字段/表单/BPF/WebResource/Plugin Assembly/Step/Custom API/App Action 等）后，必须在 DEV 验证通过的第一时间加入主清单（工具：`MetadataTool add-solution-component`，幂等）；本地临时独立 Assembly、测试组件、他人组件不得加入；组件从环境删除时须同步从主清单移除。AI 代为增删主清单组件必须获得用户明确授权。详细规则见 `/skill:d365-dev` 第 8.3.1 节，发版核对见 `/skill:d365-deploy` 第 5.1 节。
