...
var FsmDataForm = (function () {
    ...
    // 金融产品全部选项（Bug #1507 方案A）：与 mcs_fsm_resource_products / 融资资源 mcs_fsm_institution_products 同编码
    // 银行 1-11（英文标签）、保险 101-104（中文标签）
    var ALL_PRODUCT_OPTIONS = [
        { value: 1,   label: "Non-recourse Accounts Receivable Factoring" },
        { value: 2,   label: "Recourse Accounts Receivable Factoring" },
        { value: 3,   label: "Purchase Loan Financing" },
        { value: 4,   label: "Inventory Financing" },
        { value: 5,   label: "Dealer Financing" },
        { value: 6,   label: "Retail Factoring" },
        { value: 7,   label: "Leasing" },
        { value: 8,   label: "Consortium" },
        { value: 9,   label: "Investment Loan" },
    ...
    /**
     * 融资机构编号变更时自动带出机构名称（Bug #1507，PRD：融资机构编号选择后自动带出）
     */
    function syncResourceName(formContext) {
        var nameAttr = formContext.getAttribute("mcs_fsm_resource_name");
        if (!nameAttr) return;
        var resourceRef = getLookup(formContext, "mcs_fsm_resource_id");
        if (!resourceRef) {
            if (nameAttr.getValue() !== null) nameAttr.setValue(null); // 清空机构时同步清空名称
            return;
        }
        retrieveSafe("mcs_fsm_resource", resourceRef.id, "?$select=mcs_fsm_institution_name").then(function (r) {
            var institutionName = r ? r["mcs_fsm_institution_name"] : null;
            if (nameAttr.getValue() !== institutionName) nameAttr.setValue(institutionName);
        });
    }

    /**
     * 金融产品（多选）下拉选项按所选融资机构已维护的产品过滤（Bug #1507 方案A，PRD：下拉选择将融资机构金融产品带入选择项）
     * 未选机构 → 清空可选项（无可选）；已选机构 → 仅保留该机构 mcs_fsm_institution_products 勾选的项
     * @param pruneSelection 仅机构变更时为 true：剔掉已选值中不在新机构产品范围内的项；onLoad 不过滤已有值防静默改数据
     */
    function filterProductsByResource(formContext, pruneSelection) {
        var ctrl = formContext.getControl("mcs_fsm_resource_products");
        if (!ctrl || !ctrl.clearOptions) return;
        var attr = formContext.getAttribute("mcs_fsm_resource_products");
        var resourceRef = getLookup(formContext, "mcs_fsm_resource_id");
        if (!resourceRef) {
            ctrl.clearOptions();
            return;
        }
        retrieveSafe("mcs_fsm_resource", resourceRef.id, "?$select=mcs_fsm_institution_products").then(function (r) {
            if (!r) return; // 查询失败（网络抖动等）保持现有选项不变，避免误清空
            var raw = r["mcs_fsm_institution_products"];
            var allowed = {};
            if (raw) {
                String(raw).split(",").forEach(function (x) {
                    var v = parseInt(x, 10);
                    if (!isNaN(v)) allowed[v] = true;
                });
            }
            ctrl.clearOptions();
            ALL_PRODUCT_OPTIONS.forEach(function (o) {
                // MultiSelectOptionSet 控件 addOption 只接受 option 对象（{text,value}），不接受 (label, value) 两参数
                if (allowed[o.value]) ctrl.addOption({ text: o.label, value: o.value });
            });
            if (pruneSelection && attr) {
                var current = attr.getValue();
                if (current && current.length > 0) {
                    var kept = current.filter(function (v) { return allowed[v]; });
                    if (kept.length !== current.length) attr.setValue(kept);
                }
            }
        });
    }
    ...
    var REQUIRED_SOLUTION_EXTRA = [
        "mcs_fsm_resource_products", "mcs_fsm_credit_amount",
    ...
    var SOLUTION_TAB = {
        ...
        required: ["mcs_fsm_resource_products", "mcs_fsm_credit_amount", "mcs_fsm_credit_amount_usd",
    ...
    // 融资需求区 + 融资六要素字段（Bug #1509）：阶段 1（融资需求）/2（融资立项）可写，
    // 进入阶段 3（融资解决方案）起锁定只读；状态 4 / 审批中随 lockAll 全锁。
    // 注：设备台数/产品名称已由 INITIATION_TAB_METADATA_REQUIRED 控制，不在此重复；
     ...
     *  - 融资需求区 + 融资六要素：仅状态 1/2 可写，状态 ≥3 或审批中锁定
     */
    function applyStageControl(formContext) {
        ...
        // 融资需求区 + 融资六要素：仅融资需求/融资立项阶段可写，进入融资解决方案起锁定（Bug #1509）
        var demandEditable = !lockAll && (status === FSM_STATUS.DEMAND || status === FSM_STATUS.INITIATION);
        DEMAND_AREA_FIELDS.forEach(function (f) {
            setFieldDisabled(formContext, f, !demandEditable);
        });
    }
    ...
    self.onLoad = function (executionContext) {
        ...
        // Bug #1507：融资机构编号 → 机构名称自动带出（含存量记录机构有值但名称为空的回填）
        var resourceAttr = formContext.getAttribute("mcs_fsm_resource_id");
        if (resourceAttr) resourceAttr.addOnChange(function () {
            syncResourceName(formContext);
            filterProductsByResource(formContext, true);
        });
        var resourceNameAttr = formContext.getAttribute("mcs_fsm_resource_name");
        if (resourceAttr && resourceAttr.getValue() && resourceNameAttr && !resourceNameAttr.getValue()) {
            syncResourceName(formContext);
        }
        filterProductsByResource(formContext, false);
