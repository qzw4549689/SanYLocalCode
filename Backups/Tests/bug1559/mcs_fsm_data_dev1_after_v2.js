...
var FsmDataForm = (function () {
    ...
    function applyStageControl(formContext) {
        ...
        // Bug #1559：融资资源机构多选 picker（HTML WebResource）可编辑状态同步——
        // 仅状态 3（融资解决方案）且非审批中可编辑；推+拉双通道（iframe 加载时序不保证）
        var pickerEditable = !lockAll && (status === FSM_STATUS.SOLUTION);
        try {
            window.__fsmResourcePickerEditable = pickerEditable;
            var pickerCtrl = formContext.getControl("WebResource_FsmResourcePicker");
            if (pickerCtrl && pickerCtrl.getObject) {
                var pickerWin = pickerCtrl.getObject() && pickerCtrl.getObject().contentWindow;
                if (pickerWin && pickerWin.setEditable) pickerWin.setEditable(pickerEditable);
            }
        } catch (e) { console.warn("[FSM] picker 可编辑状态同步失败:", e); }
    }
    ...
    self.onLoad = function (executionContext) {
        ...
        // 存量回填：机构多选有值但名称字段为空时重新带入（不触发过滤提示之外的写脏）
