...
class Program
{
    ...
    static string BuildConnectionStringForUrl(string url)
    {
    ...
    static async Task<ServiceClient> CreateServiceClientWithDeviceCodeAsync(string url, string appId, string? tenantId)
    {
        ...
        // 配置持久化 token 缓存，避免每次运行都重新 Device Code 登录
        ...
        // 使用 Token Provider 方式，直接把 MSAL token 传给 ServiceClient
    ...
    static async Task Main(string[] args)
    {
        ...
        try
        {
            using (ServiceClient service = await CreateServiceClientAsync(url))
            {
                if (!service.IsReady)
                {
                    ...
                    case "rebuild-scoring-cards":
                        {
                            ...
                            // DEV1 连接
                            var devCs = BuildConnectionStringForUrl("https://dev1.crm5.dynamics.com");
    ...
    {
        var names = new[] { "BPP_WorkFlowTemplateCode", "D365BaseUrl", "Bpp_ApprovalFlowBaseUrl" };
