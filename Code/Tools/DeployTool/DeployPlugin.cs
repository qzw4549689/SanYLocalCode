...
namespace DeployTool
{
    public class DeployPlugin
    {
        ...
        public static void UpdateMainSalesAssembly(ServiceClient service)
        {
            Console.WriteLine(">>> 更新主 Plugin Assembly SanyD365.D365Extension.Sales...");
            ...
            if (!File.Exists(dllPath))
            {
                Console.WriteLine($"  ❌ Plugin DLL 不存在: {dllPath}");
                Console.WriteLine("     请先用 scp 把远程编译好的 DLL 拉到 /tmp/SanyD365.D365Extension.Sales.dll");
                return;
            }
            ...
            try
            {
                ...
                if (assemblyId == Guid.Empty)
                {
                    Console.WriteLine("  ❌ 主 Plugin Assembly 更新失败");
                    return;
                }

                Console.WriteLine("  ✅ 主 Plugin Assembly 更新完成");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"  ❌ 主 Plugin Assembly 更新失败: {ex.Message}");
            }
        }
