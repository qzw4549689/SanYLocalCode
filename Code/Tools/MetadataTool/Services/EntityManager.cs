...
namespace D365MetadataTool
{
    ...
    public class EntityManager
    {
        ...
        public void UpdateOptionLabels(string entityName, string fieldName, Dictionary<int, string> valueToLabel)
        {
            ...
            foreach (var opt in valueToLabel)
            {
                try
                {
                    Console.WriteLine($"  更新选项 {opt.Key} -> {opt.Value}");
                    var request = new UpdateOptionValueRequest
                    {
                        EntityLogicalName = entityName,
                        AttributeLogicalName = fieldName,
                        Value = opt.Key,
                        Label = LabelHelper.Create(opt.Value)
                    };
                    _service.Execute(request);
    ...
    <event name='onsave' application='false' active='true'>
      <Handlers>
        ...
        public void CreateQualitativeEnumRecords()
        {
            ...
            {
                // CountryRisk 国别风险
                // Coface 返回 A1/A2/A3/A4/B/C/D/E，按业务规则映射到低/中/高风险
                ...
                // SectorRisk 行业风险
                // Coface 返回 1/2/3/4，按业务规则映射到低/中/高风险
                ("SectorRisk", "1", "低风险", 5),
                ("SectorRisk", "2", "中风险", 3),
                ("SectorRisk", "3", "高风险", -1),
                ("SectorRisk", "4", "高风险", -1),
                ...
                // Sectors 行业属性
                // NACE 映射后的三一行业中文名称，listValue 与 NACE 配置表 mcs_coface_nace_mapping.mcs_sanyindustry 保持一致
                ("Sectors", "矿业", "矿业", 5),
                ("Sectors", "港务", "港务", 4),
                ("Sectors", "建工", "建工", 4),
                ("Sectors", "吊装", "吊装", 4),
            ...
            foreach (var en in enums)
            {
                ...
                if (result.Entities.Count > 0)
                {
                    var existingRecord = result.Entities[0];
                    var existingName = existingRecord.GetAttributeValue<string>("mcs_listname");
                    if (existingName != en.listName)
                    {
                        var update = new Entity("mcs_credititem_value", existingRecord.Id)
                        {
                            ["mcs_listname"] = en.listName
                        };
                        _service.Update(update);
                        Console.WriteLine($"  ↻ 更新名称: {en.itemCode}/{en.listValue} => {en.listName}");
                        updated++;
                    }
                    else
                    {
            ...
            Console.WriteLine($"\n完成: 创建{created}条, 更新{updated}条, 已存在{existing}条, 跳过{skipped}条, 总计{created + updated + existing + skipped}条");
        }
        ...
        /// 3. 调用 CreateQualitativeEnumRecords 创建/更新正确数据
        /// </summary>
        public void FixCofaceQualitativeEnums()
        {
            ...
            // 需要删除的旧记录
            var toDelete = new List<(string itemCode, string listValue)>
            {
                // CountryRisk 旧错误数据（Coface 返回 A1/A2/A3/A4/B/C/D/E，不是 1/2/3）
                ("CountryRisk", "1"),
                ("CountryRisk", "2"),
                ("CountryRisk", "3"),
                
                // Sectors 旧英文数据（改为中文 listValue）
                ("Sectors", "Mining"),
                ("Sectors", "Port"),
            ...
            int deleted = 0;
            foreach (var (itemCode, listValue) in toDelete)
            {
                if (!itemGuidMap.TryGetValue(itemCode, out Guid itemGuid))
                {
                    Console.WriteLine($"  ⚠️ 未找到评分项目 {itemCode}，跳过删除 {listValue}");
                    continue;
                }
                ...
                if (result.Entities.Count > 0)
                {
                    ...
                    try
                    {
                        ...
                        Console.WriteLine($"  ✓ 删除旧记录: {itemCode}/{listValue}");
                        deleted++;
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"  ❌ 删除失败 {itemCode}/{listValue}: {ex.Message}");
                    }
                }
                else
                {
                    Console.WriteLine($"  ⊘ 不存在: {itemCode}/{listValue}");
                }
            }
