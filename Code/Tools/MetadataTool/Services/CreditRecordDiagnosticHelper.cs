...
public class CreditRecordDiagnosticHelper
{
    ...
    {
        Console.WriteLine($"\n=== 按客户名称/编码诊断信用评估记录: {accountName} ===");
        Console.WriteLine($"环境: {_service.ConnectedOrgUriActual}\n");

        // 先按名称查，再按 accountnumber / mcs_sapnumber 查
        Entity? account = null;
        var accountQuery = new QueryExpression("account")
        {
            ColumnSet = new ColumnSet("accountid", "name", "accountnumber", "mcs_sapnumber"),
            Criteria = new FilterExpression
            {
        ...
        {
            account = accounts.Entities[0];
        }
        else
        {
            accountQuery = new QueryExpression("account")
            {
                ColumnSet = new ColumnSet("accountid", "name", "accountnumber", "mcs_sapnumber"),
                Criteria = new FilterExpression
                {
                    Conditions =
                    {
                        new ConditionExpression("accountnumber", ConditionOperator.Equal, accountName),
                        new ConditionExpression("mcs_sapnumber", ConditionOperator.Equal, accountName)
                    },
                    FilterOperator = LogicalOperator.Or
                }
            };
            accounts = _service.RetrieveMultiple(accountQuery);
            if (accounts.Entities.Count > 0)
            {
                account = accounts.Entities[0];
            }
        ...
        var foundName = account.GetAttributeValue<string>("name");
        Console.WriteLine($"找到客户: {foundName} ({accountId})");
    ...
    private void DiagnoseRecord(Entity record)
    {
        ...
        if (accountRef != null)
        {
            ...
            if (masterRef != null)
            {
                ...
                foreach (var card in scoringCards.Entities)
                {
                    ...
                    var dataType = card.GetAttributeValue<OptionSetValue>("mcs_datatype")?.Value ?? 1;
                    var minValue = card.GetAttributeValue<decimal?>("mcs_minvalue");
                    var maxValue = card.GetAttributeValue<decimal?>("mcs_maxvalue");
                    var weight = card.GetAttributeValue<int>("mcs_weight");
                    var listValue = ReadListValue(card);

                    string rangeText;
                    if (dataType == 1 || dataType == 100000000)
                    {
                        rangeText = $"[{minValue?.ToString("F2") ?? "null"}, {maxValue?.ToString("F2") ?? "null"})";
                    }
                    else
                    {
                        rangeText = $"listvalue={listValue ?? "(空)"}";
                    }

                    Console.WriteLine($"   - {itemName} ({itemCode}) | 类型={dataType}, 权重={weight}, {rangeText}");
                }
        ...
        try
        {
            var tagQuery = new QueryExpression("mcs_customer_tag")
            {
                ColumnSet = new ColumnSet(
                    ...
                    "mcs_itemvalue2", "mcs_scorevalue", "mcs_isscore"
                ),
                Criteria = new FilterExpression
                {
            ...
            foreach (var tag in tags.Entities)
            {
                ...
                PrintField(tag, "mcs_itemtxtvalue2", "  复核定性: ");
                PrintField(tag, "mcs_itemintvalue2", "  复核定量: ");
                PrintField(tag, "mcs_itemvalue2", "  复核指标: ");
                PrintField(tag, "mcs_scorevalue", "  得分值: ");
                PrintField(tag, "mcs_isscore", "  是否评分: ");
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            var traceQuery = new QueryExpression("plugintracelog")
            {
                ...
                Orders = { new OrderExpression("createdon", OrderType.Descending) },
                PageInfo = { Count = 50, PageNumber = 1 }
            };
        ...
        // 8. 查询 CreditScoreCalculationPlugin Trace 日志
        Console.WriteLine("\nCreditScoreCalculationPlugin 最近 Trace 日志:");
        try
        {
            var traceQuery = new QueryExpression("plugintracelog")
            {
            ...
            var allTraces = _service.RetrieveMultiple(traceQuery).Entities
                .Where(t => t.GetAttributeValue<string>("typename")?.Contains("CreditScoreCalculationPlugin") == true)
                .Take(10)
                .ToList();
            Console.WriteLine($"找到 {allTraces.Count} 条 CreditScoreCalculationPlugin Trace 日志");
            foreach (var trace in allTraces)
            {
                var createdOn = trace.GetAttributeValue<DateTime>("createdon");
                var message = trace.GetAttributeValue<string>("messageblock");
                var typeName = trace.GetAttributeValue<string>("typename");
                Console.WriteLine($"\n  [{createdOn:yyyy-MM-dd HH:mm:ss}] {typeName}");
                Console.WriteLine($"  {message?.Substring(0, Math.Min(message?.Length ?? 0, 1500))}...");
            }
        }
        catch (Exception ex)
        {
    ...
    private string? ReadListValue(Entity card)
    {
        if (!card.Contains("mcs_listvalue") || card["mcs_listvalue"] == null)
            return null;

        var value = card["mcs_listvalue"];
        if (value is EntityReference refValue)
        {
            try
            {
                var lvEntity = _service.Retrieve("mcs_credititem_value", refValue.Id, new ColumnSet("mcs_listvalue", "mcs_listname"));
                var lv = lvEntity.GetAttributeValue<string>("mcs_listvalue");
                var ln = lvEntity.GetAttributeValue<string>("mcs_listname");
                return string.IsNullOrEmpty(lv) ? refValue.Name : $"{lv} ({ln})";
            }
            catch
            {
