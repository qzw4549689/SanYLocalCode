        ...
        public async Task<BPPFormData> GetBppFormData(Guid EntityId, string EntityName)
        {
            try
            {
              ...
              <entity name=""mcs_fca_quotaapp"">
                ...
                // 查询客户主数据获取客户编码（mcs_sapnumber）与客户名称（mcs_name）
                // 注意：mcs_fca_quotaapp.mcs_accountid 直接指向 mcs_customermasterdata（非 account）
                // Bug #1271 修复：原逻辑误用该 GUID 查 account 导致客户编码为空、客户名称显示成编码
                string accountCode = string.Empty;
                string customerName = record.GetLookupEntityReference("mcs_accountid")?.Name ?? string.Empty;
                ...
                if (accountRef != null)
                {
                  ...
                  <entity name=""mcs_customermasterdata"">
                    ...
                    <filter type=""and"">
                      <condition attribute=""mcs_customermasterdataid"" operator=""eq"" value=""{accountRef.Id}""/>
                    </filter>
                  </entity>
                </fetch>";
                    ...
                    var cmdName = cmd?.GetStringValue("mcs_name");
                    if (!string.IsNullOrWhiteSpace(cmdName))
                    {
                        customerName = cmdName;
                    }
                    LoggerMainHelper.LogInformation($"BPPHandlerServiceForFcaQuotaApp.GetBppFormData 查询客户编码", $"AccountCode={accountCode}, CustomerName={customerName}");
                }
                // 客户名称兜底：客户主数据取不到时回退为记录上的 mcs_custname
                if (string.IsNullOrWhiteSpace(customerName))
                {
                    customerName = record.GetStringValue("mcs_custname") ?? string.Empty;
                }
                ...
                LoggerMainHelper.LogInformation($"BPPHandlerServiceForFcaQuotaApp.GetBppFormData 组装表单数据", $"GrantId={grantId}, AccountCode={accountCode}, CustomerName={customerName}");
