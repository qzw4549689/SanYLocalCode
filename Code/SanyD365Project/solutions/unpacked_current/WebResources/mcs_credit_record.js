...
/**
 * 初始化附件页签
 * 向通用上传组件传入当前评估记录(mcs_credit_record)上下文
 */
CreditRecordForm.initAttachmentTab = function (formContext) {
    ...
    // 通过 D365 API 获取 WebResource 控件对应的 iframe DOM 元素
    var iframe = null;
    if (typeof uploaderControl.getObject === "function") {
        iframe = uploaderControl.getObject();
    }
    if (!iframe) {
        // 兜底：尝试通过 document 查找
        var el = document.getElementById("mcs_credit_record_uploader");
    ...
    // 获取基础 URL（移除现有 data 参数）
    var currentSrc = iframe.getAttribute("src") || "";
    var baseUrl = currentSrc.split("?")[0] || "/WebResources/mcs_/CommonCore/Html/Uploader.html";
    ...
    if (recordId && recordId !== "00000000-0000-0000-0000-000000000000") {
        ...
        iframe.src = baseUrl + "?data=" + encodeURIComponent(JSON.stringify(data));
    } else {
        // 新建未保存记录时没有 ID，保持空组件
        iframe.src = baseUrl;
    }
