...
using SanyD365.D365Extension.Sales.Application.Sales.CofaceIntegration;
using SanyD365.D365Extension.Sales.Application.Sales.CofaceIntegration.Api;
using SanyD365.D365Extension.Sales.Application.Sales.CofaceIntegration.Parser;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text.Json;

namespace SanyD365.D365Extension.Sales.Plugins.CofaceIntegration
{
    /// <summary>
    /// Coface数据集成Plugin
    ...
    /// </summary>
    public class CofaceIntegrationDataSyncPlugin : PluginBase
    {
...
tracer.Trace("===== CofaceDataSyncPlugin 开始执行 =====");
        ...
        private void UpdateTagValue(IOrganizationService service, ITracingService tracer, Guid tagId, int dataType, object value, string itemCode)
        {
            ...
            else // 定性
            {
                ...
                // 解析并写入复核定性指标 Lookup（mcs_credititem_value）
                var tagRecord = service.Retrieve("mcs_customer_tag", tagId, new ColumnSet("mcs_credit_item"));
                var creditItemRef = tagRecord.GetAttributeValue<EntityReference>("mcs_credit_item");
                Guid? enumId = ResolveCreditItemValue(service, tracer, creditItemRef, stringValue, displayValue);
                if (enumId.HasValue)
                {
                    updateTag["mcs_credititem_value"] = new EntityReference("mcs_credititem_value", enumId.Value);
                    tracer.Trace($"写入定性Lookup: {enumId.Value}");
                }
                else
                {
                    tracer.Trace($"未找到定性枚举映射: item={creditItemRef?.Id}, raw={stringValue}, display={displayValue}");
                }
