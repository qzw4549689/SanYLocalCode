...
// 订单信息提取公共逻辑已抽取到 CofaceOrderInfoHelper（供数据集成与下单插件复用），此处用别名保持原引用不变
using UrbaOrderStatus = SanyD365.Plugins.CofaceIntegration.CofaceOrderInfoHelper.UrbaOrderStatus;
using UrbaOrderInfo = SanyD365.Plugins.CofaceIntegration.CofaceOrderInfoHelper.UrbaOrderInfo;
using ReportOrderStatus = SanyD365.Plugins.CofaceIntegration.CofaceOrderInfoHelper.ReportOrderStatus;
using ReportOrderInfo = SanyD365.Plugins.CofaceIntegration.CofaceOrderInfoHelper.ReportOrderInfo;
...
namespace SanyD365.Plugins.CofaceIntegration.Plugin
{
    ...
    public class CofaceDataSyncPlugin : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            // 系统上下文：Coface 基础配置数据读取（NACE 行业映射/财务指标对照/标准汇率）
            // 不依赖操作员权限，这 3 类基础数据实体计划对业务用户收回 Read 权限
            ...
            try
            {
                ...
                Entity creditRecord = service.Retrieve("mcs_credit_record", target.Id,
                    new ColumnSet("mcs_scoreid", "mcs_accountid", "mcs_custnameen", "mcs_countrycode", "mcs_cofaceid", "ownerid"));
                ...
                // 初始化API服务
                // Parser 内部仅读取基础配置数据（financial_indicator/nace_mapping/transactioncurrency），
                // 统一使用系统上下文，不依赖操作员对基础数据实体的权限
                var apiService = new CofaceApiService(service, tracer);
                var urbaParser = new Urba360Parser(tracer, systemService, countryCode);
                var reportParser = new FullReportParser(tracer, systemService);
                ...
                // ========== 步骤2.5: 获取内部历史交易数据 ==========
                tracer.Trace("步骤2.5: 获取内部历史交易数据");
                var internalData = GetInternalTransactionData(service, systemService, tracer, creditRecord);
                tracer.Trace($"内部历史交易指标: {internalData.Count}个");
                ...
                foreach (var kvp in internalData) allData[kvp.Key] = kvp.Value;
            ...
            out UrbaOrderStatus orderStatus)
        {
            ...
            try
            {
                ...
                using (var ordersDoc = apiService.GetUrbaMonitoringOrders(cofaceId, countryCode))
                {
                    ...
                    var orderInfo = CofaceOrderInfoHelper.ExtractUrbaOrderInfo(ordersDoc, tracer);
        ...
        /// <summary>
        /// 获取CRM内部历史交易数据
        /// - SalesAmount: 汇总销售订单 totalamount_base（基础货币，已折算为USD）
        /// - ARAmount: 最近24个月内 mcs_outstanding 逾期记录的最大逾期金额（人民币→USD）
        /// - ARAge: 最近24个月内 mcs_outstanding 逾期记录的最大逾期天数
        /// </summary>
        private Dictionary<string, object> GetInternalTransactionData(
            IOrganizationService service,
            ...
            Entity creditRecord)
        {
            ...
            try
            {
                ...
                {
                    tracer.Trace("评估记录未关联客户，跳过内部历史交易数据获取");
                    return result;
                }
                ...
                if (salesAmount > 0)
                {
                    ...
                    tracer.Trace($"SalesAmount={salesAmount:F2}");
                }
                else
                {
                    tracer.Trace("SalesAmount=0或无效，不写入");
                }

                // ARAmount & ARAge: 从 mcs_outstanding 获取
                var (arAmount, arAge) = GetOutstandingData(service, systemService, tracer, accountId.Value);
                if (arAmount > 0)
                {
                    result["ARAmount"] = arAmount;
                    tracer.Trace($"ARAmount={arAmount:F2}");
                }
                else
                {
                    tracer.Trace("ARAmount=0或无效，不写入");
                }

                if (arAge > 0)
                {
                    result["ARAge"] = arAge;
                    tracer.Trace($"ARAge={arAge}");
                }
                else
                {
                    tracer.Trace("ARAge=0或无效，不写入");
                }
            }
            catch (Exception ex)
            {
                tracer.Trace($"获取内部历史交易数据异常: {ex.Message}");
            }
        ...
        /// <summary>
        /// 从 mcs_outstanding 获取最近24个月内的最大逾期金额（USD）和最大逾期天数
        /// </summary>
        private (decimal arAmount, int arAge) GetOutstandingData(IOrganizationService service, IOrganizationService systemService, ITracingService tracer, Guid accountId)
        {
            decimal maxOverdueAmount = 0;
            ...
            try
            {
                ...
                {
                    ColumnSet = new ColumnSet("mcs_overdueamount", "mcs_overdurationdays", "createdon", "mcs_createon"),
                    Criteria = new FilterExpression
                    {
                        Conditions =
                        {
                            new ConditionExpression("mcs_account", ConditionOperator.Equal, accountId),
                            new ConditionExpression("mcs_overdue", ConditionOperator.Equal, true)
                        }
                    }
                };
                ...
                {
                    // 判断生成版本时间：优先 mcs_createon，fallback createdon
                    DateTime? createdOn = record.GetAttributeValue<DateTime?>("mcs_createon");
                    if (!createdOn.HasValue)
                    {
                        createdOn = record.GetAttributeValue<DateTime?>("createdon");
                    }

                    if (!createdOn.HasValue || createdOn.Value < cutoffDate)
                    {
                        continue;
                    }

                    counted++;
                    var overdueAmount = record.GetAttributeValue<decimal?>("mcs_overdueamount") ?? 0;
                    var overdueDays = record.GetAttributeValue<int?>("mcs_overdurationdays") ?? 0;
                ...
                tracer.Trace($"mcs_outstanding 逾期统计: 客户={accountId}, 24个月内记录数={counted}, 最大逾期金额(CNY)={maxOverdueAmount:F2}, 最大逾期天数={maxOverdueDays}");

                // 逾期金额人民币 → USD
                if (maxOverdueAmount > 0)
                {
                    // 汇率读取走系统上下文（transactioncurrency 标准汇率）
                    var rate = CofaceExchangeRateHelper.GetRateToUsd(systemService, tracer, "CNY");
                    if (rate > 0)
                    {
                        maxOverdueAmount = Math.Round(maxOverdueAmount * rate, 2);
                        tracer.Trace($"逾期金额汇率转换: CNY→USD, rate={rate}, 转换后={maxOverdueAmount:F2}");
                    }
                    else
                    {
                        tracer.Trace("CNY→USD 汇率获取失败，逾期金额保持人民币");
                    }
                }
            }
            catch (Exception ex)
            {
                tracer.Trace($"获取 mcs_outstanding 逾期数据异常: {ex.Message}");
            }

            return (maxOverdueAmount, maxOverdueDays);
        }
                ...
                try
                {
                    ...
                    if (tagId.HasValue)
                    {
                            ...
                            && value != null)
                        {
                            ...
                            {
                                tracer.Trace($"指标 {itemCode} 小数两位转换失败: {ex.Message}");
                            }
                        }
            ...
            try
            {
                ...
                foreach (var record in records.Entities)
                {
                    ...
                    if (string.IsNullOrEmpty(itemCode))
                    {
                        ...
                        if (itemRef != null)
                        {
                            var item = service.Retrieve("mcs_credit_items", itemRef.Id, new ColumnSet("mcs_credit_itemsno", "mcs_group", "mcs_itemdesc"));
                            itemCode = item?.GetAttributeValue<string>("mcs_credit_itemsno") ?? "";
                            itemDesc = item?.GetAttributeValue<string>("mcs_itemdesc") ?? "";
                        }
                    }
                    else
                    {
                        ...
                        if (itemRef != null)
                        {
                            var item = service.Retrieve("mcs_credit_items", itemRef.Id, new ColumnSet("mcs_itemdesc"));
                            itemDesc = item?.GetAttributeValue<string>("mcs_itemdesc") ?? "";
                        }
                    }
            ...
            int dataType)
        {
            ...
            // 标签负责人跟随评估记录负责人，避免业务员看不到由系统/管理员上下文生成的标签
            if (creditRecord.Contains("ownerid") && creditRecord["ownerid"] is EntityReference recordOwner)
            {
                newTag["ownerid"] = recordOwner;
                tracer.Trace($"设置标签负责人为评估记录负责人: {recordOwner.Id}");
            }
        ...
        /// 复核字段默认与集成字段保持一致，但仅当复核字段当前无值时写入（首次赋值），避免覆盖人工复核结果
        /// </summary>
        ...
        {
            // 查询当前标签复核字段，判断是否已经存在复核值
            var currentTag = service.Retrieve("mcs_customer_tag", tagId,
                new ColumnSet("mcs_itemintvalue2", "mcs_itemtxtvalue2", "mcs_itemvalue2", "mcs_credititem_value", "mcs_credit_item"));

            bool reviewValueExists = false;
            if (dataType == 1) // 定量：int2 与 value2 同时有有效值才算已赋值
            {
                reviewValueExists = currentTag.Contains("mcs_itemintvalue2") && currentTag["mcs_itemintvalue2"] != null
                    && currentTag.Contains("mcs_itemvalue2") && currentTag.GetAttributeValue<string>("mcs_itemvalue2") != "N/A";
            }
            else // 定性：txt2 与 lookup 同时有有效值才算已赋值
            {
                reviewValueExists = currentTag.Contains("mcs_itemtxtvalue2") && currentTag["mcs_itemtxtvalue2"] != null
                    && currentTag.Contains("mcs_credititem_value") && currentTag["mcs_credititem_value"] != null;
            }

            tracer.Trace($"UpdateTagValue: tagId={tagId}, dataType={dataType}, reviewValueExists={reviewValueExists}");
            ...
            if (dataType == 1) // 定量
            {
                ...
                // 复核定量指标为空时，默认赋上集成值（只有第一次）
                if (!reviewValueExists)
                {
                    if (value == null)
                    {
                        updateTag["mcs_itemintvalue2"] = null;
                        updateTag["mcs_itemvalue2"] = "N/A";
                        tracer.Trace("首次赋值复核定量指标: N/A");
                    }
                    else
                    {
                        decimal decimalValue = Convert.ToDecimal(value);
                        ...
                        {
                            updateTag["mcs_itemintvalue2"] = 99999999999m;
                            updateTag["mcs_itemvalue2"] = "99999999999.00";
                            tracer.Trace("首次赋值复核定量指标: 99999999999.00 (超出范围截断)");
                        }
                        else
                        {
                            updateTag["mcs_itemintvalue2"] = decimalValue;
                            updateTag["mcs_itemvalue2"] = decimalValue.ToString("F2");
                            tracer.Trace($"首次赋值复核定量指标: {decimalValue:F2}");
                        }
                    }
                }
            }
            else // 定性
            {
                ...
                // 若未传入 Lookup，按评分项目+三一编码/显示名反查定性枚举值
                if (creditItemValueRef == null)
                {
                    var creditItemRef = currentTag.GetAttributeValue<EntityReference>("mcs_credit_item");
                    if (creditItemRef != null)
                    {
                        creditItemValueRef = ResolveCreditItemValue(service, tracer, creditItemRef, sanyValue, displayValue);
                        if (creditItemValueRef != null)
                        {
                            tracer.Trace($"反查定性枚举Lookup: {creditItemValueRef.Id}");
                        }
                    }
                }
                ...
                tracer.Trace($"更新定性值: Coface原始={rawValue}, 三一编码={sanyValue}, 显示={displayValue}");

                // 复核定性指标为空时，默认赋上集成值（只有第一次）
                if (!reviewValueExists)
                {
                    updateTag["mcs_itemtxtvalue2"] = displayValue;
                    updateTag["mcs_itemvalue2"] = sanyValue;
                    if (creditItemValueRef != null)
                    {
                        updateTag["mcs_credititem_value"] = creditItemValueRef;
                    }
                    tracer.Trace($"首次赋值复核定性指标: Coface原始={rawValue}, 三一编码={sanyValue}, 显示={displayValue}");
                }
            }
        ...
        /// <summary>
        /// 根据评分项目Lookup和原始值/显示名反查 mcs_credititem_value 记录
        /// 优先按 mcs_listvalue（原始值）匹配，其次按 mcs_listname（显示名）匹配
        /// </summary>
        private EntityReference ResolveCreditItemValue(
            IOrganizationService service,
            ITracingService tracer,
            EntityReference creditItemRef,
            string listValue,
            string displayValue)
        {
            try
            {
                ...
                {
                    orFilter.Conditions.Add(new ConditionExpression("mcs_listvalue", ConditionOperator.Equal, listValue));
                    orFilter.Conditions.Add(new ConditionExpression("mcs_listname", ConditionOperator.Equal, listValue));
                }
                ...
                enumQuery.Criteria.AddFilter(orFilter);

                var enumResult = service.RetrieveMultiple(enumQuery);
                if (enumResult.Entities.Count > 0)
                
                {
                    return new EntityReference("mcs_credititem_value", enumResult.Entities[0].Id);
                }
            }
            catch (Exception ex)
            {
                tracer.Trace($"ResolveCreditItemValue 异常: {ex.Message}");
            }
