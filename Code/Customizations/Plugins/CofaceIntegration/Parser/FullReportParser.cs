        ...
        private decimal? ParseRegisteredCapital(JsonElement creditReport)
        {
            try
            {
                ...
                if (creditReport.TryGetProperty("shareCapital", out var shareCapital))
                {
                    ...
                    {
                        decimal? value = issuedAmount.GetDecimalSafe();
                        if (value.HasValue)
                        {
                            decimal convertedValue = ApplyDimension(shareCapital, value.Value);
                            convertedValue = ConvertCurrency(shareCapital, convertedValue);
                    ...
                    {
                        decimal? value = paidUpAmount.GetDecimalSafe();
                        if (value.HasValue)
                        {
                            decimal convertedValue = ApplyDimension(shareCapital, value.Value);
                            convertedValue = ConvertCurrency(shareCapital, convertedValue);
                    ...
                    company.TryGetProperty("capital", out var capital))
                {
                    ...
                    {
                        decimal? value = fromAmount.GetDecimalSafe();
                        if (value.HasValue)
                        {
                            // 处理dimension和货币转换
                            decimal convertedValue = ApplyDimension(indicator, value.Value);
                            convertedValue = ConvertCurrency(indicator, convertedValue);
