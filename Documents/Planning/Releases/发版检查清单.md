...
- [ ] **2.5** **生产直连字段终核（2026-09-01 新增，Frank 生产只读账号）**：用 Frank 账号（`gw_zhangf68`，密码向用户索取，env 临时传入）直连生产 `list-fields` 读取本次涉及实体的生产字段类型，与发版包字段 diff——🚨 同名字段类型不一致必须=0；**可替代「请三一生成产全量包对比」**，分实体轻量抽核（上班时间禁止全量扫描）。连接方式与红线见 `/skill:d365-deploy` 第 5.5 节
